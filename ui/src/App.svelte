<script lang="ts">
  ...
  import { nav, goTo } from "./lib/stores";
  ...
  onMount(async () => {
    await initSettings();
    if (isTray) return;
    // The global hotkey raises the window straight into the task manager.
    // App-lifetime listener — no teardown needed.
    void api.onSummon(() => goTo("taskmgr"));
